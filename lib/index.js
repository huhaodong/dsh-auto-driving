...
const Config = z.object({
	...
	/**
	 * Per-conversation mode overrides (会话隔离): keys are session ids; each
	 * entry pins THAT conversation's model-fallback ("fallback") and/or
	 * full-auto ("auto") switch. A boolean pins the mode for that conversation
	 * only — one conversation's toggle never moves another's; null or a missing
	 * key follows the global default above. Pins only apply while the global
	 * master switch of that mode is on: the settings-page switch is the
	 * availability gate (off → the capsule half disappears everywhere), a stale
	 * pin cannot resurrect a globally disabled mode. The composer pill writes
	 * here through POST /dsh-model-fallback/api/session-state, and subagent
	 * sessions inherit their root conversation's entry via the parentSession walk.
	 */
	sessionModes: z
		.dict(
			z.object({
				auto: z.union([z.boolean(), z.const(null)]).default(null),
				fallback: z.union([z.boolean(), z.const(null)]).default(null),
			}),
		)
		.default({}),
	/**
...
function apply(ctx, config) {
	...
	//#region per-conversation mode overrides (会话隔离开关)
	/**
	 * sessionId -> { auto, fallback }: the per-conversation toggles. A boolean
	 * pins that conversation's mode regardless of the global default; absent
	 * keys (or nulls) follow it. Written by the composer pill through
	 * /api/session-state; the {@link sessionOverrideOf} walk lets subagent
	 * sessions inherit their root conversation's entry.
	 */
	const sessionModes = new Map();
	/** Canonical JSON of the overrides we last persisted ourselves — our own commit echoes are not external edits. */
	let storedModesJson = "";

	/** Canonicalize a stored sessionModes dict: sorted keys, typed booleans, both-null entries dropped. */
	function canonicalModesObject(stored) {
		const out = {};
		const entries = Object.entries(stored && typeof stored === "object" ? stored : {}).sort(([a], [b]) => (a < b ? -1 : a > b ? 1 : 0));
		for (const [id, entry] of entries) {
			if (typeof id !== "string" || id.length === 0 || entry === null || typeof entry !== "object") continue;
			const auto = typeof entry.auto === "boolean" ? entry.auto : null;
			const fallback = typeof entry.fallback === "boolean" ? entry.fallback : null;
			if (auto === null && fallback === null) continue;
			out[id] = { auto, fallback };
		}
		return out;
	}

	/** Replace the live override map from a canonical stored dict (external settings edits only). */
	function seedSessionModes(canonical) {
		sessionModes.clear();
		for (const [id, modes] of Object.entries(canonical)) sessionModes.set(id, { ...modes });
	}

	/** Persist the live override map into the plugin's settings section (best-effort). */
	function persistSessionModes() {
		if (!settingsService || typeof settingsService.update !== "function") return;
		const stored = canonicalModesObject(Object.fromEntries(sessionModes));
		storedModesJson = JSON.stringify(stored);
		settingsService
			.update(NS, { sessionModes: stored })
			.catch((error) => logger.warn(`model-fallback: failed to persist per-session modes: ${error?.message ?? error}`));
	}

	/** Apply one POSTed mode write ({auto?, fallback?} of boolean|null) and persist it. */
	function writeSessionMode(sessionId, patch) {
		const modes = { ...(sessionModes.get(sessionId) ?? { auto: null, fallback: null }) };
		for (const key of ["auto", "fallback"]) {
			if (patch[key] === undefined) continue;
			modes[key] = typeof patch[key] === "boolean" ? patch[key] : null;
		}
		if (modes.auto === null && modes.fallback === null) {
			// Both modes back to "follow global" — drop the entry entirely.
			sessionModes.delete(sessionId);
			if (settingsService && typeof settingsService.mutate === "function") {
				settingsService
					.mutate(NS, [{ op: "unset", path: ["sessionModes", sessionId] }])
					.catch((error) => logger.warn(`model-fallback: failed to clear per-session modes for ${sessionId}: ${error?.message ?? error}`));
			}
			return;
		}
		sessionModes.set(sessionId, modes);
		persistSessionModes();
	}

	/** Normalize an optional session id so lookups stay type-safe. */
	function sessionIdOf(value) {
		return typeof value === "string" && value.length > 0 ? value : undefined;
	}

	/**
	 * Resolve one mode for a session: the nearest explicit override on the
	 * session itself or any `parentSession` ancestor wins (so subagent requests
	 * inherit the root conversation's pill state); null falls back to the
	 * global settings default.
	 */
	function sessionOverrideOf(sessionId, mode) {
		let id = sessionIdOf(sessionId);
		for (let hops = 0; id !== undefined && hops < 16; hops += 1) {
			const entry = sessionModes.get(id);
			const value = entry ? entry[mode] : undefined;
			if (typeof value === "boolean") return value;
			const session = sessionsService && typeof sessionsService.get === "function" ? sessionsService.get(id) : undefined;
			const parent = session?.header?.parentSession;
			id = typeof parent === "string" && parent.length > 0 && parent !== id ? parent : undefined;
		}
		return null;
	}

	/**
	 * Effective model-fallback switch for one conversation. The settings-page
	 * master switch is the availability gate (关闭后胶囊按钮从所有对话消失):
	 * while it is off NO conversation can run the loop — a stale per-conversation
	 * pin cannot resurrect a globally disabled mode, mirroring the client where
	 * the capsule half disappears. While it is on, the conversation's own pin
	 * wins over the global default (会话隔离).
	 */
	function fallbackEnabledFor(sessionId) {
		if (current()?.enabled !== true) return false;
		const override = sessionOverrideOf(sessionId, "fallback");
		return override !== null ? override : true;
	}

	/** Effective full-auto switch for one conversation (same global gate + per-conversation pin). */
	function autoEnabledFor(sessionId) {
		if (autoFlag("enabled", false) !== true) return false;
		const override = sessionOverrideOf(sessionId, "auto");
		return override !== null ? override : true;
	}
	//#endregion
	/**
	...
	function buildChain(options) {
		...
		// 会话隔离: the requesting conversation's own toggle wins over the global
		// default — turning the fallback off in one conversation must not touch
		// any other, and vice versa.
		if (fallbackEnabledFor(requestSessionId(options)) !== true) {
			if (provider.length > 0) {
				logDecision(`skip:${provider}`, `model-fallback: not engaged for ${provider} (${cfg.enabled === true ? "fallback disabled for this conversation" : "fallback globally disabled"})`);
			}
			return null;
		}
	...
	function scheduleGoalAutoResume(agent, code) {
		try {
			...
			const cfg = current();
			// 会话隔离: the conversation's own fallback toggle governs auto-resume.
			if (cfg?.enabled !== true) return;
			if (fallbackEnabledFor(sessionIdOf(agent.id)) !== true) return;
	...
	ctx.on("agent/request-error", async (payload, next) => {
		...
		const cfg = current();
		// 会话隔离: the conversation's own fallback toggle governs loop-level
		// recovery too (the config source is re-read at every event).
		if (cfg?.enabled !== true) return outcome;
		if (fallbackEnabledFor(sessionObjectSessionId(session) ?? sessionIdOf(agent?.id)) !== true) return outcome;
	...
	function injectTurnKeepAlive(agent, code, state, failure, maxTokens) {
		...
		try {
			...
			const cfg = current();
			// 会话隔离: the conversation's own fallback toggle governs the wake too.
			if (!cfg || cfg.enabled !== true || fallbackEnabledFor(sessionIdOf(agent.id)) !== true || cfg?.retry?.keepAlive?.enabled === false) return;
	...
	ctx.on("agent/error", ({ agent, error }) => {
		try {
			const cfg = current();
			// 会话隔离: the conversation's own fallback toggle governs keep-alive.
			if (!cfg || cfg.enabled !== true || fallbackEnabledFor(sessionIdOf(agent?.id)) !== true) return;
			const ka = cfg?.retry?.keepAlive;
	...
	ctx.on("session/event", (session, event) => {
		try {
			...
			if (kind === "max-tokens") {
				const cfg = current();
				const ka = cfg?.retry?.keepAlive;
				if (fallbackEnabledFor(session.id) === true && ka?.enabled !== false && state.timer === null && !keepAliveConsecutiveExhausted(state, ka?.maxConsecutive)) {
	...
	try {
		ctx.inject(["webServer"], (scope) => {
			scope.effect(() => {
				...
				/** Whether a request arrives from the loopback interface (settings-write trust level). */
				const isLoopbackRequest = (request) => {
					const remote = request?.socket?.remoteAddress;
					return remote === "127.0.0.1" || remote === "::1" || remote === "::ffff:127.0.0.1";
				};

				/** One JSON snapshot of a conversation's toggle state for the client pill. */
				const sessionStatePayload = (sessionId) => {
					const entry = sessionModes.get(sessionId);
					return {
						sessionId,
						modes: {
							auto: typeof entry?.auto === "boolean" ? entry.auto : null,
							fallback: typeof entry?.fallback === "boolean" ? entry.fallback : null,
						},
						effective: {
							auto: autoEnabledFor(sessionId) === true,
							fallback: fallbackEnabledFor(sessionId) === true,
						},
						global: {
							auto: autoFlag("enabled", false) === true,
							fallback: current()?.enabled === true,
						},
					};
				};

				// Per-conversation toggle state (会话隔离): read one conversation's
				// pinned modes, or write them from the composer pill. Writes carry
				// the same trust level as settings writes: loopback only.
				const disposeSessionState = scope.webServer.register({
					kind: "exact",
					path: "/dsh-model-fallback/api/session-state",
					handler: (request, response) => {
						if (request.method === "GET" || request.method === "HEAD") {
							let sessionId;
							try {
								sessionId = sessionIdOf(new URL(request.url ?? "/", "http://localhost").searchParams.get("sessionId"));
							} catch {
								sessionId = undefined;
							}
							if (!sessionId) {
								response.writeHead(400, { "content-type": "application/json; charset=utf-8", "cache-control": "no-store" });
								response.end(JSON.stringify({ error: "sessionId query parameter required" }));
								return;
							}
							response.writeHead(200, { "content-type": "application/json; charset=utf-8", "cache-control": "no-store" });
							response.end(request.method === "HEAD" ? undefined : JSON.stringify(sessionStatePayload(sessionId)));
							return;
						}
						...
						if (!isLoopbackRequest(request)) {
							...
							response.end(JSON.stringify({ error: "loopback connections only" }));
							return;
						}
						let raw = "";
						let oversized = false;
						request.on("data", (chunk) => {
							raw += chunk;
							if (raw.length > 16384) oversized = true;
						});
						request.on("error", () => {});
						request.on("end", () => {
							try {
								...
								for (const key of ["auto", "fallback"]) {
									...
									if (typeof value !== "boolean" && value !== null) throw new TypeError(`"${key}" must be a boolean or null`);
									patch[key] = value;
								}
								if (!sessionId || (patch.auto === undefined && patch.fallback === undefined)) {
									response.writeHead(400, { "content-type": "application/json; charset=utf-8" });
									response.end(JSON.stringify({ error: "sessionId and at least one of auto/fallback (boolean|null) required" }));
									return;
								}
								writeSessionMode(sessionId, patch);
								logger.info(`model-fallback: conversation ${sessionId} toggles updated (auto: ${JSON.stringify(patch.auto ?? "unchanged")}, fallback: ${JSON.stringify(patch.fallback ?? "unchanged")})`);
								response.writeHead(200, { "content-type": "application/json; charset=utf-8", "cache-control": "no-store" });
								response.end(JSON.stringify(sessionStatePayload(sessionId)));
							} catch (error) {
								response.writeHead(400, { "content-type": "application/json; charset=utf-8" });
	...
	function describeSelection(cfg) {
		...
		return `${cfg?.enabled === true ? "enabled" : "disabled"}; ${selected.length} provider group(s)${selected.length > 0 ? `: ${selected.join(" -> ")}` : ""}; protectUnselected=${cfg?.protectUnselected === false ? "off" : "on"}; allProvidersFallback=${cfg?.allProvidersFallback === false ? "off" : "on"}; arrears=${arrearsKeys.length > 0 ? arrearsKeys.join(",") : "none"}; per-conversation toggles=${sessionModes.size}`;
	}
	...
	installSettingsSection(ctx, NS, Config, config ?? {}, {
		...
		onChange: () => {
			const cfg = current();
			// Per-conversation overrides: external settings edits (another
			// process, a hand-edited file) reseed the live map. Our own
			// persistence echoes carry the exact canonical JSON we wrote, so
			// they are recognized and skipped instead of clobbering in-flight
			// pill writes.
			const canonical = canonicalModesObject(cfg?.sessionModes);
			const canonicalJson = JSON.stringify(canonical);
			if (canonicalJson !== storedModesJson) {
				storedModesJson = canonicalJson;
				seedSessionModes(canonical);
			}
	...
	ctx.on("approval/request", (req, next) => {
		// 会话隔离: the asking conversation's own autopilot toggle decides —
		// other conversations keep their own state.
		const sessionIdent = sessionObjectSessionId(req?.agent?.session) ?? sessionIdOf(req?.agent?.id);
		if (!autoEnabledFor(sessionIdent) || !autoFlag("autoAllowPermissions", true)) return next();
	...
	if (userQuestions && typeof userQuestions.registerProvider === "function" && !userQuestions.autoModeWrapped) {
		...
		userQuestions.registerProvider = (provider) => originalRegister({
			ask: async (request) => {
				...
				// 会话隔离: the asking conversation's own autopilot toggle decides.
				const sessionIdent = sessionObjectSessionId(request?.agent?.session) ?? sessionIdOf(request?.agent?.id);
				const gatedOff =
					!autoEnabledFor(sessionIdent) ||
	...
	// 3. Model-facing notice so the agent stops waiting for a human. The
	// assembly hands the rendering agent to the text function, so the notice
	// follows THAT conversation's own autopilot toggle (session isolation).
	try {
		ctx.inject(["systemPrompt"], (scope) => {
			scope.systemPrompt.context({
				name: "auto-mode:enabled",
				order: 116,
				text: (assembly) => (autoEnabledFor(sessionObjectSessionId(assembly?.agent?.session) ?? sessionIdOf(assembly?.agent?.id)) ? AUTO_PROMPT_SENTENCE : ""),
			});
		});
	} catch (error) {
