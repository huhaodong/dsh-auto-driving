window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			`.dshmfb-retryTextarea{min-height:64px;resize:vertical;line-height:1.5}`,
			`.dshmfb-retryTextarea:disabled{opacity:.5}`,
			...
			// ── responsive compact: a tight or content-squeezed composer row
			// drops the text labels and keeps the bot / layers icons only
			// (class toggled by the squeeze-aware observer) ──
			`.dshmfb-comboPill.dshmfb-comboCompact .dshmfb-halfLabel{display:none}`,
			`.dshmfb-comboPill.dshmfb-comboCompact .dshmfb-halfPill{padding:0 10px}`,
			...
			// ── sticky toolbar: sub-menu left, page actions right ──
			`.dshmfb-toolbar{position:sticky;top:0;z-index:20;display:flex;align-items:center;gap:10px;flex-wrap:wrap;background:var(--dsw-alias-bg-layer-1,#171a21);margin:-6px 0 -4px;padding:10px 0 12px;border-bottom:1px solid var(--mfb-border)}`,
			`.dshmfb-toolbarActions{margin-left:auto;display:inline-flex;align-items:center;gap:8px;flex-wrap:wrap}`,
			// ── toast bubble (README mockup style: amber pill, single instance) ──
			`.dshmfb-toastHost{position:fixed;z-index:20000;pointer-events:none;display:flex;flex-direction:column;align-items:center;transform:translateX(-50%)}`,
			`.dshmfb-toast{display:inline-flex;align-items:center;gap:8px;padding:8px 16px;border-radius:999px;background:rgba(224,162,76,.12);border:1px solid rgba(224,162,76,.45);color:#ecc387;font-size:12px;line-height:1.5;box-shadow:0 8px 24px rgba(0,0,0,.28);opacity:0;transform:translateY(-8px);transition:opacity .2s ease,transform .2s ease;max-width:min(420px,calc(100vw - 48px));pointer-events:auto;overflow-wrap:anywhere}`,
			`.dshmfb-toastShow .dshmfb-toast{opacity:1;transform:translateY(0)}`,
			`.dshmfb-toastDot{width:7px;height:7px;border-radius:999px;background:#e0a24c;box-shadow:0 0 6px rgba(224,162,76,.8);flex:none}`,
		...
		const zh = {
			...
			watchdog: "活性看门狗",
			keepAlive: "轮次保活（继续唤醒）",
			keepAliveDesc: "整轮任务因模型服务异常而失败（所有候选切换、同模型重试、循环重试都已耗尽，即「本轮运行失败」出现时），自动向对话发送一条「继续」，让任务接着跑，直到成功完成。仅在失败属于可重试类型、且模型池未全部欠费时触发；连续失败按指数退避，成功后自动复位。",
			keepAliveDelay: "首次唤醒延迟(ms)",
			keepAliveDelayHint: "轮次失败后等待多久发送第一条「继续」；连续失败每次翻倍",
			keepAliveMaxDelay: "退避上限(ms)",
			keepAliveMaxDelayHint: "「继续」间隔的上限；长时间故障下仍保持约每分钟一次探测，直到模型池恢复或全部欠费",
			keepAliveBackoff: "退避倍率",
			keepAliveBackoffHint: "连续失败时唤醒间隔的倍率（2 = 5s → 10s → 20s…）",
			keepAliveMaxConsecutive: "最大连续次数",
			keepAliveMaxConsecutiveHint: "同一会话连续自动「继续」的上限；0 = 不限（一直尝试直到成功或模型池全部欠费）",
			keepAliveGuard: "幂等护栏",
			keepAliveGuardHint: "发送「继续」前检查上一步工具调用：结果未确认或已成功时附加指引，提示模型先确认状态、不要重复执行副作用操作",
			keepAliveContinueText: "继续文本",
			keepAliveContinueTextHint: "中断后自动发送的文本；支持 {code} {message} {status} {tool} {turn} {errorCount} {elapsed} 占位符，留空用默认「继续」",
			keepAliveMaxTokensText: "超限时的继续文本",
			keepAliveMaxTokensTextHint: "达到输出 token 上限时自动发送的文本（同样支持占位符）；留空用默认「继续」",
			keepAlivePatterns: "自定义可恢复错误",
			keepAlivePatternsHint: "每行一个普通文本片段（不区分大小写）；命中错误码、HTTP 状态或消息时强制视为可恢复，优先于内置分类（参考 dsh-auto-continue 的 retryableErrorPatterns）",
			...
			autoPill: "自动驾驶",
			fallbackPill: "模型回退",
			fallbackPillOn: "模型自动回退（仅当前对话）：当前模型在产出任何内容前失败时，自动切换到候选池中的下一个模型，任务不中断。\n当前对话已开启 · 点击关闭（其他对话不受影响）",
			fallbackPillOff: "模型自动回退（仅当前对话）：当前模型在产出任何内容前失败时，自动切换到候选池中的下一个模型，任务不中断。\n当前对话已关闭 · 点击开启（其他对话不受影响）",
			fallbackPillBusy: "正在切换，请稍候…",
			switchNotify: "模型已切换: {from} → {to}",
			switchRecovered: "已在 {model} 恢复执行",
			switchSyncFail: "模型选择同步失败（会话可能不支持）",
			autoPillOn: "自动驾驶（全自动模式，仅当前对话）：任务中的权限审批、确认提问、方案评审全部按默认允许自动应答，无需等待人工。\n当前对话已开启 · 点击关闭（其他对话不受影响）",
			autoPillOff: "自动驾驶（全自动模式，仅当前对话）：任务中的权限审批、确认提问、方案评审全部按默认允许自动应答，无需等待人工。\n当前对话已关闭 · 点击开启（其他对话不受影响）",
			autoPillBusy: "正在切换，请稍候…",
			autoPlanNotice: "已自动批准方案",
			autoAnsweredNotice: "已自动应答确认提问",
			autoAllowedNotice: "已自动允许权限",
			stateOn: "开",
			stateOff: "关",
			globalDefaultHint: "以上开关是所有对话的全局总闸兼默认值：关闭某项后，输入框胶囊上对应的按钮将从所有对话中消失（两项都关闭则整个胶囊不再显示）；开启时每个对话可在胶囊上单独开关（会话隔离，互不影响）。",
			logTitle: "工作日志",
		...
		const en = {
			...
			keepAlive: "Turn keep-alive (continue wake-up)",
			keepAliveDesc: "When a whole turn dies to a transient model-service failure — every candidate switch, same-model retry, and loop retry spent, i.e. the \"This turn failed\" boundary — automatically send a \"continue\" message into the conversation so the task restarts and keeps going until it succeeds. Fires only for retryable failure classes while the model pool is not entirely in arrears; consecutive failures back off exponentially and the streak resets on success.",
			keepAliveDelay: "First wake-up delay (ms)",
			keepAliveDelayHint: "How long to wait after a dead turn before sending the first \"continue\"; doubles per consecutive failure",
			keepAliveMaxDelay: "Backoff cap (ms)",
			keepAliveMaxDelayHint: "Upper bound on the interval between \"continue\" messages; a long outage still probes about once a minute until the pool recovers or every account is in arrears",
			keepAliveBackoff: "Backoff multiplier",
			keepAliveBackoffHint: "Wake-up interval multiplier per consecutive failure (2 = 5s → 10s → 20s…)",
			keepAliveMaxConsecutive: "Max consecutive continues",
			keepAliveMaxConsecutiveHint: "Ceiling on consecutive auto-continues per session; 0 = unlimited (keep trying until success or the whole pool is in arrears)",
			keepAliveGuard: "Idempotency guard",
			keepAliveGuardHint: "Before sending \"continue\", inspect the last tool call: when its result is unconfirmed or it completed successfully, append guidance telling the model to verify state first and not re-run side-effecting tools",
			keepAliveContinueText: "Continue text",
			keepAliveContinueTextHint: "Text sent after an interruption; supports {code} {message} {status} {tool} {turn} {errorCount} {elapsed} placeholders; blank uses the default \"continue\"",
			keepAliveMaxTokensText: "Max-tokens continue text",
			keepAliveMaxTokensTextHint: "Text sent when the output-token ceiling is reached (same placeholders); blank uses the default \"continue\"",
			keepAlivePatterns: "Custom retryable errors",
			keepAlivePatternsHint: "One literal fragment per line (case-insensitive); any match against the error code, HTTP status, or message forces the failure to count as recoverable, overriding built-in classification (mirrors dsh-auto-continue's retryableErrorPatterns)",
			...
			fallbackPillOn: "Model fallback (this conversation only): when the current model fails before producing any output, the request switches to the next candidate automatically, so the task keeps going.\nCurrently ON for this conversation · click to turn off (other conversations are unaffected)",
			fallbackPillOff: "Model fallback (this conversation only): when the current model fails before producing any output, the request switches to the next candidate automatically, so the task keeps going.\nCurrently OFF for this conversation · click to turn on (other conversations are unaffected)",
			...
			autoPillOn: "Auto-drive (full-auto, this conversation only): permission approvals, confirmation questions, and plan reviews are answered automatically during tasks — no waiting for a human.\nCurrently ON for this conversation · click to turn off (other conversations are unaffected)",
			autoPillOff: "Auto-drive (full-auto, this conversation only): permission approvals, confirmation questions, and plan reviews are answered automatically during tasks — no waiting for a human.\nCurrently OFF for this conversation · click to turn on (other conversations are unaffected)",
			autoPillBusy: "switching…",
			autoPlanNotice: "Auto-approved plan",
			autoAnsweredNotice: "Auto-answered confirmation",
			autoAllowedNotice: "Auto-allowed permission",
			stateOn: "ON",
			stateOff: "OFF",
			globalDefaultHint: "The switches above are the global master gates and defaults for every conversation: turning one off removes its half of the composer capsule in every conversation (both off hides the capsule entirely); while on, each conversation can toggle its own state on the capsule (per-conversation isolation).",
		...
		const DEFAULT_RETRYABLE_CODES = [
			...
			"RATE_LIMITED",
			"SERVER_ERROR",
			// pi-ai adapter vocabulary + its catch-all (mirrors the host list).
			"SERVER",
			"RATE_LIMIT",
			"PI_AI_ERROR",
			"500",
		...
		function RetrySection({ controller, t, writable }) {
			...
			const baseKeepAlive = stored.keepAlive && typeof stored.keepAlive === "object" ? stored.keepAlive.enabled !== false : true;
			const baseKeepAliveDelay = stored.keepAlive && typeof stored.keepAlive === "object" && typeof stored.keepAlive.delayMs === "number" ? stored.keepAlive.delayMs : 5000;
			const baseKeepAliveMax = stored.keepAlive && typeof stored.keepAlive === "object" && typeof stored.keepAlive.maxDelayMs === "number" ? stored.keepAlive.maxDelayMs : 60000;
			const baseKeepAliveBackoff = stored.keepAlive && typeof stored.keepAlive === "object" && typeof stored.keepAlive.backoffFactor === "number" ? stored.keepAlive.backoffFactor : 2;
			const baseKeepAliveMaxConsecutive = stored.keepAlive && typeof stored.keepAlive === "object" && typeof stored.keepAlive.maxConsecutive === "number" ? stored.keepAlive.maxConsecutive : 0;
			const baseKeepAliveGuard = stored.keepAlive && typeof stored.keepAlive === "object" ? stored.keepAlive.guardTools !== false : true;
			const baseKeepAliveContinueText = stored.keepAlive && typeof stored.keepAlive === "object" && typeof stored.keepAlive.continueText === "string" ? stored.keepAlive.continueText : "";
			const baseKeepAliveMaxTokensText = stored.keepAlive && typeof stored.keepAlive === "object" && typeof stored.keepAlive.continueTextMaxTokens === "string" ? stored.keepAlive.continueTextMaxTokens : "";
			const baseKeepAlivePatterns = stored.keepAlive && typeof stored.keepAlive === "object" && typeof stored.keepAlive.retryablePatterns === "string" ? stored.keepAlive.retryablePatterns : "";

			const [draft, setDraft] = react.useState(null);
			const [saving, setSaving] = react.useState(false);
			const [error, setError] = react.useState(null);

			/** Draft shape: { enabled, maxRetries, baseDelayMs, loopRetry, watchdog, watchdogIdleMs, watchdogResends, keepAlive, keepAliveDelayMs, keepAliveMaxDelayMs, keepAliveBackoff, keepAliveMaxConsecutive, keepAliveGuard, keepAliveContinueText, keepAliveMaxTokensText, keepAlivePatterns, codesText }. */
			const eff = draft ?? { enabled: baseEnabled, maxRetries: baseMax, baseDelayMs: baseDelay, loopRetry: baseLoopRetry, watchdog: baseWatchdog, watchdogIdleMs: baseWatchdogIdle, watchdogResends: baseWatchdogResends, keepAlive: baseKeepAlive, keepAliveDelayMs: baseKeepAliveDelay, keepAliveMaxDelayMs: baseKeepAliveMax, keepAliveBackoff: baseKeepAliveBackoff, keepAliveMaxConsecutive: baseKeepAliveMaxConsecutive, keepAliveGuard: baseKeepAliveGuard, keepAliveContinueText: baseKeepAliveContinueText, keepAliveMaxTokensText: baseKeepAliveMaxTokensText, keepAlivePatterns: baseKeepAlivePatterns, codesText: baseCodes.join(", ") };
			...
			const dirty =
				...
				(draft.enabled !== baseEnabled ||
					...
					draft.keepAlive !== baseKeepAlive ||
					draft.keepAliveDelayMs !== baseKeepAliveDelay ||
					draft.keepAliveMaxDelayMs !== baseKeepAliveMax ||
					draft.keepAliveBackoff !== baseKeepAliveBackoff ||
					draft.keepAliveMaxConsecutive !== baseKeepAliveMaxConsecutive ||
					draft.keepAliveGuard !== baseKeepAliveGuard ||
					draft.keepAliveContinueText !== baseKeepAliveContinueText ||
					draft.keepAliveMaxTokensText !== baseKeepAliveMaxTokensText ||
					draft.keepAlivePatterns !== baseKeepAlivePatterns ||
			...
			const save = async () => {
				...
				try {
					await controller.set("retry", {
						...
						keepAlive: {
							enabled: draft.keepAlive !== false,
							delayMs: Math.max(0, Math.floor(draft.keepAliveDelayMs) || 5000),
							maxDelayMs: Math.max(0, Math.floor(draft.keepAliveMaxDelayMs) || 60000),
							backoffFactor: Math.max(1, Math.floor(draft.keepAliveBackoff) || 2),
							maxConsecutive: Math.max(0, Math.floor(draft.keepAliveMaxConsecutive) || 0),
							guardTools: draft.keepAliveGuard !== false,
							continueText: (draft.keepAliveContinueText ?? "").trim(),
							continueTextMaxTokens: (draft.keepAliveMaxTokensText ?? "").trim(),
							retryablePatterns: (draft.keepAlivePatterns ?? "").trim(),
						},
			...
			const restoreDefaults = () => {
				setDraft({ enabled: true, maxRetries: 3, baseDelayMs: 500, loopRetry: true, watchdog: true, watchdogIdleMs: 300000, watchdogResends: 2, keepAlive: true, keepAliveDelayMs: 5000, keepAliveMaxDelayMs: 60000, keepAliveBackoff: 2, keepAliveMaxConsecutive: 0, keepAliveGuard: true, keepAliveContinueText: "", keepAliveMaxTokensText: "", keepAlivePatterns: "", codesText: DEFAULT_RETRYABLE_CODES.join(", ") });
			};
			...
			return h(
				...
				h(
					...
					h(
						...
						h("input", {
							type: "checkbox",
							checked: eff.keepAlive !== false,
							disabled: !writable,
							onChange: (event) => setField("keepAlive", event.target.checked),
						}),
						t("keepAlive"),
					),
					h("p", { className: "dshmfb-retryDesc" }, t("keepAliveDesc")),
				),
				h(
					"div",
					{ className: "dshmfb-retryField" },
					h(
						"div",
						{ className: "dshmfb-retryFieldHead" },
						h("span", { className: "dshmfb-retryLabel" }, t("keepAliveDelay")),
						h("input", {
							className: "dshmfb-retryInput",
							type: "number",
							min: "0",
							step: "500",
							value: eff.keepAliveDelayMs,
							disabled: !writable || eff.keepAlive === false,
							onChange: (event) => setField("keepAliveDelayMs", Math.max(0, parseInt(event.target.value, 10) || 0)),
						}),
					),
					h("p", { className: "dshmfb-retryHint" }, t("keepAliveDelayHint")),
				),
				h(
					"div",
					{ className: "dshmfb-retryField" },
					h(
						"div",
						{ className: "dshmfb-retryFieldHead" },
						h("span", { className: "dshmfb-retryLabel" }, t("keepAliveMaxDelay")),
						h("input", {
							className: "dshmfb-retryInput",
							type: "number",
							min: "0",
							step: "1000",
							value: eff.keepAliveMaxDelayMs,
							disabled: !writable || eff.keepAlive === false,
							onChange: (event) => setField("keepAliveMaxDelayMs", Math.max(0, parseInt(event.target.value, 10) || 0)),
						}),
					),
					h("p", { className: "dshmfb-retryHint" }, t("keepAliveMaxDelayHint")),
				),
				h(
					"div",
					{ className: "dshmfb-retryField" },
					h(
						"div",
						{ className: "dshmfb-retryFieldHead" },
						h("span", { className: "dshmfb-retryLabel" }, t("keepAliveBackoff")),
						h("input", {
							className: "dshmfb-retryInput",
							type: "number",
							min: "1",
							step: "0.5",
							value: eff.keepAliveBackoff,
							disabled: !writable || eff.keepAlive === false,
							onChange: (event) => setField("keepAliveBackoff", Math.max(1, parseFloat(event.target.value) || 2)),
						}),
					),
					h("p", { className: "dshmfb-retryHint" }, t("keepAliveBackoffHint")),
				),
				h(
					"div",
					{ className: "dshmfb-retryField" },
					h(
						"div",
						{ className: "dshmfb-retryFieldHead" },
						h("span", { className: "dshmfb-retryLabel" }, t("keepAliveMaxConsecutive")),
						h("input", {
							className: "dshmfb-retryInput",
							type: "number",
							min: "0",
							step: "1",
							value: eff.keepAliveMaxConsecutive,
							disabled: !writable || eff.keepAlive === false,
							onChange: (event) => setField("keepAliveMaxConsecutive", Math.max(0, parseInt(event.target.value, 10) || 0)),
						}),
					),
					h("p", { className: "dshmfb-retryHint" }, t("keepAliveMaxConsecutiveHint")),
				),
				h(
					"div",
					{ className: "dshmfb-retryField" },
					h(
						...
						h("input", {
							type: "checkbox",
							checked: eff.keepAliveGuard !== false,
							disabled: !writable || eff.keepAlive === false,
							onChange: (event) => setField("keepAliveGuard", event.target.checked),
						}),
						t("keepAliveGuard"),
					),
					h("p", { className: "dshmfb-retryHint" }, t("keepAliveGuardHint")),
				),
				h(
					"div",
					{ className: "dshmfb-retryField" },
					h(
						"div",
						{ className: "dshmfb-retryFieldHead" },
						h("span", { className: "dshmfb-retryLabel" }, t("keepAliveContinueText")),
						h("input", {
							className: "dshmfb-retryInputWide",
							type: "text",
							value: eff.keepAliveContinueText,
							placeholder: "继续",
							spellCheck: false,
							disabled: !writable || eff.keepAlive === false,
							onChange: (event) => setField("keepAliveContinueText", event.target.value),
						}),
					),
					h("p", { className: "dshmfb-retryHint" }, t("keepAliveContinueTextHint")),
				),
				h(
					"div",
					{ className: "dshmfb-retryField" },
					h(
						"div",
						{ className: "dshmfb-retryFieldHead" },
						h("span", { className: "dshmfb-retryLabel" }, t("keepAliveMaxTokensText")),
						h("input", {
							className: "dshmfb-retryInputWide",
							type: "text",
							value: eff.keepAliveMaxTokensText,
							placeholder: "继续",
							spellCheck: false,
							disabled: !writable || eff.keepAlive === false,
							onChange: (event) => setField("keepAliveMaxTokensText", event.target.value),
						}),
					),
					h("p", { className: "dshmfb-retryHint" }, t("keepAliveMaxTokensTextHint")),
				),
				h(
					"div",
					{ className: "dshmfb-retryField" },
					h(
						"div",
						{ className: "dshmfb-retryFieldHead" },
						h("span", { className: "dshmfb-retryLabel" }, t("keepAlivePatterns")),
						h("textarea", {
							className: "dshmfb-retryInputWide dshmfb-retryTextarea",
							rows: 3,
							value: eff.keepAlivePatterns,
							placeholder: "Upstream rejected the request as invalid",
							spellCheck: false,
							disabled: !writable || eff.keepAlive === false,
							onChange: (event) => setField("keepAlivePatterns", event.target.value),
						}),
					),
					h("p", { className: "dshmfb-retryHint" }, t("keepAlivePatternsHint")),
				),
				h(
					"div",
					{ className: "dshmfb-retryField" },
					h(
						...
						h("input", {
							className: "dshmfb-retryInputWide dshmfb-retryCodesInput",
							type: "text",
							value: eff.codesText,
		...
		function AutoModeSection({ controller, t }) {
			...
			return h(
				...
				h("div", { className: "dshmfb-card" },
					...
					h("p", { className: "dshmfb-retryHint" }, t("globalDefaultHint")),
			...
			return h(
				...
				h(
					...
					tab === "groups"
						? h(
								"div",
								{ className: "dshmfb-toolbarActions" },
		...
		/** Model-directory services captured at apply() for the selector sync helper. */
		const switchServices = { modelDirectories: null };

		//#region toast bubble (per-conversation: amber pill above the composer, single instance, 30 s)
		const TOAST_MS = 30000;
		let toastHideTimer = null;
		let toastRafId = 0;
		let toastLastLeft = null;
		let toastLastTop = null;
		/** sessionId -> latest status { text, at }; switching conversations replays it while fresh. */
		const sessionStatuses = new Map();
		/** The conversation page currently on screen (null = none, e.g. the settings page). */
		let activeSessionId = null;

		function ensureToastHost() {
			if (typeof document === "undefined") return null;
			let host = document.getElementById("dshmfb-toast-host");
			if (!host) {
				host = document.createElement("div");
				host.id = "dshmfb-toast-host";
				host.className = "dshmfb-toastHost";
		...
		/**
		 * Locate the conversation composer bar behind a visible capsule pill:
		 * climb from each visible .dshmfb-comboPill to the nearest ancestor that
		 * also contains the message textarea and is composer-sized. Returns null
		 * when no conversation input is on screen (e.g. on the settings page).
		 */
		function composerBarForToast() {
			const pills = [...document.querySelectorAll(".dshmfb-comboPill")].filter((el) => el.offsetParent !== null);
			for (const pill of pills) {
				let el = pill.parentElement;
				while (el && el !== document.body) {
					if (el.querySelector("textarea")) {
						const height = el.getBoundingClientRect().height;
						if (height >= 40 && height <= 160) return el;
					}
					el = el.parentElement;
				}
			}
		...
		/** Composer-bar width below which the capsule drops its text labels. */
		const COMBO_COMPACT_AT = 640;

		/**
		 * Free space (px) the action row must offer beside a compact capsule
		 * before its text labels return — more than the labels can add, so the
		 * class cannot feed back into the layout it measures (no flapping).
		 */
		const COMBO_EXPAND_SLACK = 140;

		/** Vertical offset (px) between the capsule and the row's first group that means "the trailing group wrapped". */
		const COMBO_WRAP_AT = 12;

		/**
		 * Nearest ancestor of `el` whose subtree contains the message textarea —
		 * the composer input bar the pill lives in (unlike {@link composerBarForToast}
		 * there is no height gate: the width measurement only needs the bar).
		 */
		function composerBarOfElement(el) {
			let node = el?.parentElement ?? null;
			while (node && node !== document.body) {
				if (typeof node.querySelector === "function" && node.querySelector("textarea")) return node;
		...
		/**
		 * Squeeze-aware responsive compact mode for the capsule: watch the
		 * composer and drop the text labels whenever the space around the pill
		 * is compressed, leaving the bot / layers icons only (tooltips and aria
		 * labels keep working). Two triggers:
		 *
		 * 1. The composer bar itself is narrow (< COMBO_COMPACT_AT — narrow
		 *    window, expanded sidebar), as before.
		 * 2. The action row is squeezed BY ITS OWN CONTENT — the input bar's
		 *    model selector renders long model names up to a wide cap, and the
		 *    trailing group (capsule + model + meter + send) then wraps to a
		 *    second line: the "操作栏错位" case a bar-width threshold can never
		 *    see. Detected geometrically instead: the capsule's top drops below
		 *    the row's first group, or the trailing group overflows the line.
		 *
		 * Expansion carries hysteresis (COMBO_EXPAND_SLACK): labels return only
		 * when the row offers enough free space that adding them cannot wrap it
		 * again. Structure lookups degrade gracefully — without a resolvable
		 * row the width rule still applies. Re-finds the structure when the
		 * conversation remounts it, and returns a disposer for the cleanup.
		 */
		function attachResponsivePill(pill) {
			const ResizeObserverCtor = typeof ResizeObserver === "function" ? ResizeObserver : null;
			if (!pill || !ResizeObserverCtor) return () => {};
			let observer = null;
			let mutation = null;
			let bar = null;
			let row = null;
			...
			const setCompact = (next) => {
				...
				compact = next;
				pill.classList.toggle("dshmfb-comboCompact", next);
			};

			/**
			 * Resolve the action row (the flex container that also holds the
			 * capsule's trailing group) plus its first group ("tools") and the
			 * capsule's own group. The row is the child of the composer bar on
			 * the capsule's ancestor chain — structural, so hashed CSS class
			 * names never matter.
			 */
			const resolveRow = () => {
				row = null;
				...
				if (!node.parentElement || node.parentElement !== bar || node === pill) return;
				row = node;
				for (const child of row.children ?? []) {
					const hasPill = child !== pill && typeof child.contains === "function" && child.contains(pill);
					if (hasPill && !trailing) trailing = child;
					else if (!tools && !hasPill) tools = child;
				}
				try {
					if (mutation && row) mutation.observe(row, { childList: true, subtree: true, characterData: true });
					if (observer && row) observer.observe(row);
				} catch {}
			};

			const measure = () => {
				if (!bar || bar.isConnected === false) {
					bar = composerBarOfElement(pill);
					row = null;
					if (!bar) return;
					try {
				...
				if (!row || row.isConnected === false) resolveRow();
				// Width rule first: a narrow composer is always compact.
				if (bar.clientWidth < COMBO_COMPACT_AT) {
					setCompact(true);
					return;
				}
				// Structure unresolved (stub environments, unexpected host
				// markup): fall back to the plain width rule.
				if (!row || !tools || !trailing) {
					setCompact(false);
					return;
				}
				let pillRect;
				...
				try {
					...
					toolsRect = tools.getBoundingClientRect();
					trailingRect = trailing.getBoundingClientRect();
				} catch {
					return;
				}
				// Incomplete geometry (odd embedding environments): stay put
				// rather than guess.
				if (![pillRect.top, toolsRect.top, trailingRect.left, toolsRect.right].every((value) => typeof value === "number" && Number.isFinite(value))) return;
				// The trailing group wrapped to a second row line — the long
				// model-name misalignment. Compact until it fits again.
				if (pillRect.top - toolsRect.top > COMBO_WRAP_AT) {
					setCompact(true);
					return;
				}
				const gap = trailingRect.left - toolsRect.right;
				if (gap <= 0) {
					// Overflow inside one line (nowrap edge cases).
					setCompact(true);
					return;
				}
				if (gap >= COMBO_EXPAND_SLACK) setCompact(false);
			};
			...
			return () => {
				...
				pill.classList.remove("dshmfb-comboCompact");
			};
		}

		/**
		 * Track the composer bar every animation frame while the bubble is
		 * visible, so the pill keeps its relative spot above the input box when
		 * the layout moves (window resize, scroll, sidebar collapse, composer
		 * growth while typing). One getBoundingClientRect per frame is cheap and
		 * covers every shift source — far more reliable than resize/scroll
		 * listeners alone.
		 */
		function startToastFollow() {
			if (toastRafId || typeof requestAnimationFrame !== "function") return;
			const step = () => {
				toastRafId = 0;
				positionToast();
				toastRafId = requestAnimationFrame(step);
			};
			toastRafId = requestAnimationFrame(step);
		}

		function stopToastFollow() {
			if (toastRafId && typeof cancelAnimationFrame === "function") cancelAnimationFrame(toastRafId);
			toastRafId = 0;
			toastLastLeft = null;
			toastLastTop = null;
		}

		/** Pin the toast host centered right above the composer bar (skips redundant style writes). */
		function positionToast() {
			const host = document.getElementById("dshmfb-toast-host");
			if (!host || host.dataset.visible !== "1") return;
			const bar = composerBarForToast();
			if (!bar) {
				// The conversation left the screen — the bubble goes with it.
				hideToast();
				return;
			}
			const rect = bar.getBoundingClientRect();
			const left = Math.round(rect.left + rect.width / 2);
			const top = Math.max(8, Math.round(rect.top - 44));
			if (left === toastLastLeft && top === toastLastTop) return;
			toastLastLeft = left;
			toastLastTop = top;
		...
		/** Hide the bubble and stop following the composer. */
		function hideToast() {
			stopToastFollow();
			if (toastHideTimer) {
			...
			const host = typeof document === "undefined" ? null : document.getElementById("dshmfb-toast-host");
			if (!host) return;
			host.classList.remove("dshmfb-toastShow");
			host.dataset.visible = "";
		}

		/**
		 * Paint THE pill for the conversation on screen. `shownAt` is the event
		 * time: replays after a conversation switch only live out the remainder
		 * of the 30 s window. Identical text within 15 s only refreshes the
		 * timer instead of replaying the entrance animation.
		 */
		function renderToast(text, shownAt) {
			if (typeof document === "undefined" || !text) return;
			if (!composerBarForToast()) return; // no composer on screen — nothing to anchor to
			const host = ensureToastHost();
			const now = Date.now();
			const refreshOnly = host.dataset.text === text && now - Number(host.dataset.at || 0) < TOAST_MS / 2;
			const pill = document.createElement("div");
			pill.className = "dshmfb-toast";
			const dot = document.createElement("span");
			dot.className = "dshmfb-toastDot";
			const label = document.createElement("span");
			label.textContent = String(text);
			pill.appendChild(dot);
			pill.appendChild(label);
			host.replaceChildren(pill);
			host.dataset.text = String(text);
			host.dataset.at = String(now);
			host.dataset.visible = "1";
			toastLastLeft = null; // force a fresh position write
			positionToast();
			startToastFollow();
			if (!refreshOnly) {
			...
			host.classList.add("dshmfb-toastShow");
			clearTimeout(toastHideTimer);
			toastHideTimer = setTimeout(() => {
				toastHideTimer = null;
				hideToast();
			}, Math.max(1000, TOAST_MS - (now - shownAt)));
		}

		/**
		 * Show THE bubble for one ring event — scoped to the conversation the
		 * event belongs to. The host tags every ring entry with the sessionId of
		 * the conversation that produced it; the bubble only appears on that
		 * conversation's page and never leaks into another one. Each session's
		 * latest status is kept for the 30 s window, so switching TO a
		 * conversation whose status is still fresh replays it there, and leaving
		 * a conversation (or landing on the settings page) hides the bubble.
		 * Unattributable events (no sessionId) never surface as a bubble.
		 */
		function showToast(text, sessionId) {
			if (!text) return;
			if (typeof sessionId !== "string" || sessionId.length === 0) return;
			const at = Date.now();
			sessionStatuses.set(sessionId, { text: String(text), at });
			if (sessionStatuses.size > 32) {
				// Bound the store: entries expire with the toast window anyway.
				const cutoff = at - TOAST_MS;
				for (const [id, status] of sessionStatuses) {
					if (status.at < cutoff) sessionStatuses.delete(id);
				}
			}
			if (sessionId !== activeSessionId) return; // belongs to a background conversation
			renderToast(String(text), at);
		}

		/**
		 * Track which conversation page is on screen (the composer pill reports
		 * its session id on mount and clears it on unmount). Switching
		 * conversations hides the previous conversation's bubble immediately and
		 * replays the new conversation's own status when it is still fresh.
		 */
		function setActiveSession(sessionId) {
			...
			if (status && Date.now() - status.at < TOAST_MS) renderToast(status.text, status.at);
		}
		//#endregion

		/**
		 * Cap a bubble detail so the pill keeps the mockup's compact shape (the
		 * ring message may carry up to 160 chars of question text). The full
		 * text stays one click away in the log viewer and AUTO-MODE.md.
		 */
		function bubbleDetail(text, max = 60) {
			const line = typeof text === "string" ? text : "";
			return line.length > max ? `${line.slice(0, max)}…` : line;
		}

		/**
		 * Human-readable toast for one ring event: model switch / recovery, or a
		 * full-auto decision. The three full-auto channels read apart — plan
		 * reviews bubble as「已自动批准方案」, questions as「已自动应答确认提问」,
		 * permissions as「已自动允许权限」— mirroring the switch/recovered pair.
		 * Returns { text, model } — model is set only for switch/recovery events
		 * (the session pill uses it to sync the model selector).
		 */
		function ringEventText(entry, t) {
			...
			const autoAnsweredKind = /auto-mode: (方案审批|人工确认) auto-answered \((.+)\); logged/.exec(entry.message);
			const autoAnswered = autoAnsweredKind === null ? /auto-answered \((.+)\); logged/.exec(entry.message) : null;
			const autoPermission = /permission for "([^"]+)" auto-allowed/.exec(entry.message);
			const model = switchMatch?.[1] ?? recoveredMatch?.[1];
			if (model !== undefined) {
				const from = entry.message.match(/^(?:model-fallback: )?([^ ]+) failed/)?.[1] ?? "";
			...
			if (autoAnsweredKind) {
				const notice = autoAnsweredKind[1] === "方案审批" ? t("autoPlanNotice") : t("autoAnsweredNotice");
				return { text: `${notice}: ${bubbleDetail(autoAnsweredKind[2])}` };
			}
			if (autoAnswered) return { text: `${t("autoAnsweredNotice")}: ${bubbleDetail(autoAnswered[1])}` };
			if (autoPermission) return { text: `${t("autoAllowedNotice")}: ${bubbleDetail(autoPermission[1])}` };
			return null;
		}

		/**
		 * Global toast feed: polls the event ring independently of any session so
		 * fallback switches and full-auto decisions bubble up even when no
		 * conversation input (and thus no session pill) is mounted — e.g. while
		 * the user is on the settings page. Session pills poll faster with their
		 * own cursors; identical text within 15 s only refreshes the toast.
		 */
		const globalFeed = { started: false, source: null, pollTimer: null, lastSeenAt: null, errors: 0 };

		/** Toast one ring entry and advance the shared watermark (SSE + polls). */
		function handleRingEntry(entry, t) {
			if (entry && typeof entry.at === "string" && entry.at > (globalFeed.lastSeenAt ?? "")) globalFeed.lastSeenAt = entry.at;
			const hit = ringEventText(entry, t);
			if (hit !== null) showToast(hit.text, entry?.sessionId);
		}

		async function pollRingForToasts(t) {
			try {
				const response = await fetch("/dsh-model-fallback/api/log", { cache: "no-store" });
				if (!response.ok) return;
				const payload = await response.json();
				const events = Array.isArray(payload?.events) ? payload.events : [];
				const fresh = globalFeed.lastSeenAt === null ? [] : events.filter((entry) => entry.at > globalFeed.lastSeenAt);
				if (globalFeed.lastSeenAt === null) {
					// First poll only anchors the watermark — no historical spam.
					globalFeed.lastSeenAt = events.length > 0 ? events[0].at : new Date(0).toISOString();
					return;
				}
				if (fresh.length > 0) globalFeed.lastSeenAt = fresh[0].at;
		...
		function setPollCadence(t, ms) {
			if (globalFeed.pollTimer) clearInterval(globalFeed.pollTimer);
			globalFeed.pollTimer = setInterval(() => void pollRingForToasts(t), ms);
		}

		/**
		 * Real-time toast feed: an SSE connection to the host pushes every ring
		 * event the instant it is recorded (millisecond latency — no polling
		 * wait). A slow 30 s safety poll covers events missed during reconnect
		 * gaps (the shared watermark prevents replays); while SSE is failing the
		 * poll tightens to 6 s and becomes the permanent transport after three
		 * consecutive failures.
		 */
		function startGlobalToastFeed(t) {
			if (globalFeed.started || typeof document === "undefined") return;
			globalFeed.started = true;
			if (typeof EventSource === "undefined") {
				setPollCadence(t, 6000);
				return;
			}
			try {
				const source = new EventSource("/dsh-model-fallback/api/events");
				globalFeed.source = source;
				source.onopen = () => {
					globalFeed.errors = 0;
					setPollCadence(t, 30000); // safety net for reconnect gaps
				};
				source.onmessage = (event) => {
					try {
						handleRingEntry(JSON.parse(event.data), t);
					} catch {
						// Malformed frame — the next event re-syncs.
					}
				};
				source.onerror = () => {
					globalFeed.errors += 1;
					setPollCadence(t, 6000); // SSE down — poll until it returns
					if (globalFeed.errors >= 3) {
						source.close();
						globalFeed.source = null;
					}
				};
			} catch {
				setPollCadence(t, 6000);
		...
		/**
		 * Inline icon: Lucide "bot" — the robot-head avatar that marks the
		 * auto-drive half of the capsule (both expanded and compact states).
		 */
		...
		/**
		 * Whether one capsule half is offered at all: the mode's global settings
		 * switch is the availability gate (设置界面勾选关闭 → 该半胶囊从所有对话
		 * 消失). Known-off hides the half; an unloaded snapshot keeps rendering
		 * (the HalfPill disables itself until its own snapshot is ready) so the
		 * capsule never flickers out and back during load. `defaultOn` mirrors
		 * the namespace's schema default when the stored value is missing
		 * (fallback ships enabled, full-auto ships disabled).
		 */
		function halfVisible(snap, defaultOn) {
			if (!snap || snap.status !== "ready") return true;
			const value = snap.value && typeof snap.value === "object" ? snap.value : null;
			if (value === null) return defaultOn === true;
			return value.enabled === true;
		}

		/**
		 * One half of the combined input-bar capsule. The effective state is
		 * per-conversation (会话隔离): the session's own pinned override wins;
		 * with no override the conversation follows the global settings default.
		 * `onSetOverride(!enabled)` pins the conversation to the opposite state
		 * without touching any other conversation or the global default.
		 */
		function HalfPill({ controller, t, onKey, offKey, busyKey, icon: Icon, labelKey, override, onSetOverride }) {
			const [snap, setSnap] = react.useState(controller.getSnapshot());
			react.useEffect(() => controller.subscribe(() => setSnap(controller.getSnapshot())), [controller]);

			const globalEnabled = snap.status === "ready" && snap.value && typeof snap.value === "object" ? snap.value.enabled === true : false;
			const writable = snap.writable === true && snap.mode === "host";
			const enabled = typeof override === "boolean" ? override : globalEnabled;
			...
			const toggle = async () => {
				...
				setBusy(true);
				try {
					if (typeof onSetOverride === "function") {
						// Per-conversation mode (会话隔离): pin this conversation only.
						await onSetOverride(!enabled);
					} else {
						// No conversation bound (hero composer of a not-yet-created
						// chat): fall back to the global default the new conversation
						// will inherit.
		 ...
		 * the same row as the send button). Left half toggles full-auto (bot
		 * icon), right half toggles model fallback (layers icon); active halves
		 * light up. Both halves are PER-CONVERSATION (会话隔离): they pin the
		 * bound conversation's own mode through /api/session-state, so one
		 * conversation's toggle never moves another's. The settings-page
		 * switches are the GLOBAL GATE: a half renders only while its mode is
		 * enabled globally (关闭后对应按钮从胶囊消失), and when both are off the
		 * capsule does not render at all. Polls the event ring so model switches
		 * stay visible in the conversation: the pill also REPORTS its session id
		 * to the toast layer (which conversation page is on screen) and only
		 * syncs the model selector for switch events that belong to THIS session.
		 */
		function ModelFallbackPill({ controller, autoController, t, sessionId }) {
			const pillRef = react.useRef(null);
			// Per-conversation pinned modes (会话隔离): null = follow the global
			// default. Kept in component state and mirrored against the host's
			// /api/session-state route.
			const [modes, setModes] = react.useState({ auto: null, fallback: null });
			// Global gate snapshots (设置界面勾选): subscribed live — the settings
			// mirror re-reads on document-updated, so flipping a switch in the
			// settings page (any window) adds/removes the half here reactively.
			const [globalSnap, setGlobalSnap] = react.useState(controller.getSnapshot());
			react.useEffect(() => controller.subscribe(() => setGlobalSnap(controller.getSnapshot())), [controller]);
			const [autoGlobalSnap, setAutoGlobalSnap] = react.useState(autoController.getSnapshot());
			react.useEffect(() => autoController.subscribe(() => setAutoGlobalSnap(autoController.getSnapshot())), [autoController]);

			// Report the conversation on screen to the per-conversation toast
			// layer: bubbles show/hide/replay relative to this session id.
			react.useEffect(() => {
				if (typeof sessionId !== "string" || sessionId.length === 0) return undefined;
				setActiveSession(sessionId);
				return () => {
					// Only clear while still the active one — never clobber the
					// session a newer page already reported.
					if (activeSessionId === sessionId) setActiveSession(null);
				};
			}, [sessionId]);

			/** Pull this conversation's pinned modes from the host (no-op off a conversation page). */
			const loadModes = react.useCallback(async () => {
				if (typeof sessionId !== "string" || sessionId.length === 0) return;
				try {
					const response = await fetch(`/dsh-model-fallback/api/session-state?sessionId=${encodeURIComponent(sessionId)}`, { cache: "no-store" });
					if (!response.ok) return;
					const payload = await response.json();
					const stored = payload?.modes;
					if (!stored || typeof stored !== "object") return;
					setModes({
						auto: typeof stored.auto === "boolean" ? stored.auto : null,
						fallback: typeof stored.fallback === "boolean" ? stored.fallback : null,
					});
				} catch {
					// The route may be briefly unavailable; the next tick retries.
				}
			}, [sessionId]);

			// Keep the pinned modes fresh: one fetch on mount, then a light poll
			// so a second window (or the settings file) stays in sync. Without a
			// conversation there is nothing to poll — the halves follow the
			// global snapshot directly.
			react.useEffect(() => {
				if (typeof sessionId !== "string" || sessionId.length === 0) return undefined;
				void loadModes();
				const timer = setInterval(() => void loadModes(), 4000);
				return () => clearInterval(timer);
			}, [loadModes, sessionId]);

			/**
			 * Pin this conversation's mode: optimistic local update, then POST.
			 * Only the target conversation's entry moves — the global default and
			 * every other conversation stay untouched.
			 */
			const setMode = react.useCallback(
				async (mode, value) => {
					...
					try {
						const response = await fetch("/dsh-model-fallback/api/session-state", {
							method: "POST",
						...
						void loadModes(); // converge on the host's truth
					}
				},
				[sessionId, loadModes],
			);

			// Responsive compact mode: a tight or content-squeezed composer row
			// swaps the halves to icon-only (the label spans hide, the bot /
			// layers icons stay). Re-armed per conversation so a switch that
			// remounts the composer re-resolves the fresh layout.
			react.useEffect(() => {
				const pill = pillRef.current ?? (typeof document !== "undefined" ? document.querySelector(".dshmfb-comboPill") : null);
				return attachResponsivePill(pill);
			}, [sessionId]);

			// Switch visibility: poll the event ring; every NEW "switching to …"
			// event of THIS session syncs the session's model selector to the
			// model that actually took over (the display follows reality).
			react.useEffect(() => {
				if (typeof sessionId !== "string" || sessionId.length === 0) return undefined;
				let lastSeenAt = null;
				const poll = async () => {
					try {
						...
						for (const entry of fresh.reverse()) {
							// Toasts come from the global SSE feed (single source); the
							// session pill only keeps the model-selector sync — and only
							// for events that belong to THIS conversation.
							if (typeof entry.sessionId === "string" && entry.sessionId !== sessionId) continue;
							const hit = ringEventText(entry, t);
							if (hit === null || hit.model === undefined) continue;
							syncModelSelection(sessionId, hit.model).catch(() => {});
						}
					} catch {
						// The log route may be briefly unavailable; the next tick retries.
					}
			...
			// A bound conversation pins its own modes; the hero composer of a
			// not-yet-created chat falls back to the global toggle instead.
			const perSession = typeof sessionId === "string" && sessionId.length > 0;
			// Global availability gate (设置界面勾选): each half exists only while
			// its mode is enabled globally — with the fallback switch off the
			// layers half vanishes, with the auto switch off the bot half does,
			// and with BOTH off the capsule is not rendered at all (设置界面两个
			// 都关闭 → 输入框不出现胶囊). All hooks above already ran, so the
			// session reporting, mode polling, and toast attribution keep
			// working for the conversation even while the capsule is hidden.
			const autoVisible = halfVisible(autoGlobalSnap, false);
			const fallbackVisible = halfVisible(globalSnap, true);
			if (!autoVisible && !fallbackVisible) return null;
			return h(
				"div",
				{ ref: pillRef, className: "dshmfb-comboPill", role: "group", "aria-label": `${t("autoNav")} / ${t("nav")}` },
				autoVisible
					? h(HalfPill, {
							...
							onKey: "autoPillOn",
							offKey: "autoPillOff",
							busyKey: "autoPillBusy",
							icon: IconBot,
							labelKey: "autoPill",
							override: perSession ? modes.auto : null,
							onSetOverride: perSession ? (value) => setMode("auto", value) : undefined,
						})
					: null,
				fallbackVisible
					? h(HalfPill, {
							controller,
							t,
							onKey: "fallbackPillOn",
							offKey: "fallbackPillOff",
							busyKey: "fallbackPillBusy",
							icon: IconLayers,
							labelKey: "fallbackPill",
							override: perSession ? modes.fallback : null,
							onSetOverride: perSession ? (value) => setMode("fallback", value) : undefined,
						})
					: null,
			);
		}
		...
		function apply(ctx) {
			...
			try {
				...
				// Without these services the pill still toggles; only the selector
				// sync stays off.
			}
			...
			// Global toast feed: SSE push (real-time) with a polling fallback —
			// switch/recovery/auto-approval events reach the client instantly;
			// each bubble is scoped to the conversation that produced it (the
			// composer pill reports which conversation is on screen).
			startGlobalToastFeed(t);
		}
		//#endregion
