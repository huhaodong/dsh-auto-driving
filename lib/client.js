window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			// ── responsive compact: a tight composer row drops the text labels and
			// keeps the dot / layers icon only (class toggled by ResizeObserver) ──
		...
		/**
		 * Responsive compact mode for the capsule: watch the composer bar's width
		 * and drop the text labels when the input row gets tight (narrow window,
		 * expanded sidebar), leaving the dot / layers icon only — tooltips and
		 * aria labels keep working, so nothing is lost but space. The bar's width
		 * is driven by the window and the sidebar, never by the pill itself, so
		 * toggling the class cannot feed back into the observed width (no
		 * flapping). Re-finds the bar when the conversation remounts it, and
		...
		function attachResponsivePill(pill) {
			...
			const apply = () => {
				...
				pill.classList.toggle("dshmfb-comboCompact", bar.clientWidth < COMBO_COMPACT_AT);
			};
			observer = new ResizeObserverCtor(apply);
			apply();
			return () => {
				observer.disconnect();
		 ...
		 * Polls the event ring so model switches stay visible in the conversation:
		 * the pill also REPORTS its session id to the toast layer (which
		 * conversation page is on screen) and only syncs the model selector for
		 * switch events that belong to THIS session.
		 */
		function ModelFallbackPill({ controller, autoController, t, sessionId }) {
			...
			// Responsive compact mode: a tight composer row swaps the halves to
			// icon-only (the label spans hide, the dot / layers icon stay). The
			// ref is primary; the DOM lookup only rescues environments that never
			// attach refs (keeps the behavior testable against the DOM stub).
			react.useEffect(() => {
