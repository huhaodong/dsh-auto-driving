window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region toast bubble (README mockup style: amber pill above the composer, single instance, 30 s)
		...
		/** Pin the toast host centered right above the composer bar. */
		function positionToast() {
			...
			host.style.top = `${Math.max(8, Math.round(rect.top - 44))}px`;
		}

		function bindToastListeners() {
			if (toastListenersBound || typeof window === "undefined") return;
		...
		/**
		 * Show THE bubble: a single pill anchored above the conversation input
		 * box (README mockup style). A new toast replaces the current one (never
		 * stacks) and stays visible for 30 s; identical text within 15 s only
		 * refreshes the timer instead of replaying the entrance animation. When
		 * no conversation composer is on screen the toast is suppressed — the
		 * event stays in the ring and the activity log.
		 */
		...
		function handleRingEntry(entry, t) {
			...
			if (hit !== null) showToast(hit.text);
		}
				...
				const poll = async () => {
					try {
						...
						for (const entry of fresh.reverse()) {
							...
							// session pill only keeps the model-selector sync.
							const hit = ringEventText(entry, t);
		...
		function apply(ctx) {
			...
			// switch/recovery/auto-approval bubbles even when no session pill is
			// mounted (e.g. on the settings page).
