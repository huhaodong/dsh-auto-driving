window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			`.dshmfb-comboPill{display:inline-flex;align-items:center;height:26px;border-radius:999px;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);flex:none}`,
			`.dshmfb-comboPill:focus-within{border-color:rgba(125,219,156,.55)}`,
			`.dshmfb-halfPill{position:relative;flex:1;display:flex;align-items:center;justify-content:center;width:30px;height:100%;padding:0;background:transparent;border:none;color:var(--dsw-alias-label-tertiary);cursor:pointer}`,
			`.dshmfb-halfPill:first-child{border-radius:999px 0 0 999px}`,
			`.dshmfb-halfPill:last-child{border-radius:0 999px 999px 0}`,
			...
			`.dshmfb-halfPill::after{content:attr(data-tip);position:absolute;bottom:calc(100% + 10px);right:-8px;background:var(--dsw-alias-bg-layer-1,#1a1d24);color:var(--dsw-alias-label-primary,#e8eaf0);border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.12));border-radius:8px;padding:8px 11px;font-size:11px;font-weight:400;line-height:1.65;white-space:pre-line;text-align:left;width:max-content;max-width:300px;pointer-events:none;opacity:0;transform:translateY(3px);transition:opacity .16s ease .3s,transform .16s ease .3s;box-shadow:0 8px 24px rgba(0,0,0,.28);z-index:1000}`,
			`.dshmfb-halfPill::before{content:"";position:absolute;bottom:calc(100% + 5px);right:14px;border:5px solid transparent;border-top-color:var(--dsw-alias-bg-layer-1,#1a1d24);opacity:0;transition:opacity .16s ease .3s;pointer-events:none;z-index:1000}`,
