window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			`.dshmfb-toastHost{position:fixed;top:18px;right:18px;z-index:2147483000;pointer-events:none;display:flex;flex-direction:column;align-items:flex-end}`,
		...
		//#region toast bubble (README mockup style: amber pill, single instance, 30 s)
		...
		/**
		 * Show THE bubble: a single fixed-position pill (top-right) in the README
		 * mockup style. A new toast replaces the current one (never stacks) and
		 * stays visible for 30 s; identical text within 15 s only refreshes the
		...
		function showToast(text) {
			...
			toastHideTimer = setTimeout(() => host.classList.remove("dshmfb-toastShow"), TOAST_MS);
		}
		//#endregion
