window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 */
		const globalFeed = { started: false, lastSeenAt: null };
				...
				const poll = async () => {
					try {
						...
						for (const entry of fresh.reverse()) {
							...
							showToast(hit.text);
							if (hit.model !== undefined) syncModelSelection(sessionId, hit.model).catch(() => {});
						}
					} catch {
						// The log route may be briefly unavailable; the next tick retries.
					}
		...
		function apply(ctx) {
			...
			// Global toast feed: switch/recovery/auto-approval bubbles even when no
			// session pill is mounted (e.g. on the settings page).
			startGlobalToastPoller(t);
		}
		//#endregion
