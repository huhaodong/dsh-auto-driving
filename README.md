...
- **💨 实时气泡通知** —— 模型切换、恢复执行与全自动模式的每次自动审批（权限 / 提问 / 方案）都会在**对话页输入框上方**实时弹出气泡：宿主通过 **SSE 长连接毫秒级推送**（事件写入即到达，无需等待轮询），SSE 不可用时自动降级为间隔轮询兜底。与文档截图一致的胶囊样式，**同时只显示一个**（新通知替换旧通知）、保留 30 秒；对话页不在屏幕上时不弹窗，事件仍完整记录在「运行状态 → 工作日志」。
