...
- **🏠 主页一键开关** —— 对话框输入条（发送按钮旁）有「自动驾驶」胶囊按钮，单击即在主页直接开/关全自动模式；与设置页「全自动模式」标签页状态**实时双向同步**（双重轮询兜底），不会出现两处状态不一致。

## 🎬 界面速览

安装后会在 **设置** 界面新增两个独立标签页（排在「插件」之后）：

| 「模型回退」标签页 | 「全自动模式」标签页 |
| :---: | :---: |
| ![模型回退设置页](docs/settings-model-fallback.png) | ![全自动模式设置页](docs/settings-full-auto.png) |

| 「任务重试 + 工作日志」 | 对话中的切换可视化与通知 |
| :---: | :---: |
| ![任务重试与工作日志](docs/settings-retry-log.png) | ![对话内切换可视化](docs/chat-auto-drive.png) |
...
- **运行状态卡片**：各分组的目录模型数（0 个模型时红色警告）、链预览、「未选定分组也纳入保护」开关；
- **工作日志卡片**：插件最近 200 条介入记录（切换 / 恢复 / 循环重试 / 耗尽 / engaged 决策 / 自动允许），最新在前，支持手动刷新与 5 秒自动刷新；数据来自宿主内存环形缓冲，经 `/dsh-model-fallback/api/log` 同源接口读取（与宿主日志同源）。

## 🔄 任务重试与看门狗

**「任务重试」**是主开关正下方的独立卡片，拥有独立的「放弃修改 / 保存」按钮组；保存时通过设置通道写回 `retry` 字段，宿主端在下一个请求立即生效。输入内容即时校验（非法数值按 0 处理，错误码自动去空）。在只读连接（非本机回环）下所有控件禁用。
